...
namespace GeoLocatorAPI.Controllers
{
    ...
    public class CountriesController : ControllerBase
    {
        ...
        [HttpPost("block")]
        public async Task<IActionResult> BlockCountry([FromQuery] BlockCountryDto request)
        {
            try
            {
                /*
               ...
               var countryInfo = await _countryService.GetCountryByCodeAsync(request.CountryCode);
                ...
                //{
                //    country.ExpiryTime = DateTime.UtcNow.AddMinutes(request.DurationMinutes.Value);
                //}
                ...
                _logger.LogInformation("Country blocked successfully: {CountryCode } ",country.CountryCode);
        ...
        public async Task<IActionResult> TemporarilyBlockCountry([FromQuery] BlockCountryDto request)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(request.CountryCode))
                ...
                var country = new BlockedCountry
                {
                    ...
                    BlockedAt = DateTime.UtcNow,
                    IsTemporary = true,
                    ExpiryTime = DateTime.UtcNow.AddMinutes(request.DurationMinutes.Value)
                };
